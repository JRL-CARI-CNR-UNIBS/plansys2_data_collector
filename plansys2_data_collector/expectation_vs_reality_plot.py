...
class PlanComparisonPlotGeneratorNode(Node):
    ...
    def plot_data_from_csv(self, main_path, plans_to_compare):
        ...
        for plan_execution in plans_to_compare:
            ...
            try:
                pd_plan_execution_baseline = pd.read_csv(f'{main_path}{plan_execution}_BASELINE/exported_plan_execution_data_collection.csv')
            except Exception as e:
                self.get_logger().INFO(f'Error reading {main_path}{plan_execution}_BASELINE/exported_plan_execution_data_collection.csv')
            pd_plan_execution['baseline_plan_duration'] = pd_plan_execution_baseline['plan_duration']
            pd_plan_execution['baseline_nominal_duration'] = pd_plan_execution_baseline['nominal_duration']
